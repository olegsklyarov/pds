...
if (isset($_SESSION['admin'])) {
    if (strcmp($_SESSION['md5md5login'], md5($GLOBALS['md5_admin_login'])) == 0 &&
        ...
        $tpl->assign("paramstr", student_get_students);
        ...
        if (isset($_GET[student_get_students])) {
            ...
            $tpl->assign("url", $_SERVER['SCRIPT_NAME']);
            $tpl->assign("paramstr", student_get_students . "&" . student_get_add);
            $tpl->assign("caption", "Добавить студента");

            $tpl->newBlock("menu_students");
            $tpl->assign("url", $_SERVER['SCRIPT_NAME']);
            $tpl->assign("paramstr", student_get_students . "&mail_to_all");
            ...
            if (count($lines) > 0) {
                ...
                for ($i = 0; $i < count($lines); $i++) {
                    if (isset($_GET[student_get_edit]) && $lines[$i]['id'] == $_GET[student_get_edit]) {
                        ...
                        $tpl->assign("chpass",
                            "<a href=\"" . $_SERVER['SCRIPT_NAME'] . "?" . student_get_students . "&mail_to=" . $lines[$i]['id'] . "\">chpass</a>");
                        $tpl->assign("edit_row",
                            "<a href=\"" . $_SERVER['SCRIPT_NAME'] . "?" . student_get_students . "&" . student_get_edit . "=" . $lines[$i]['id'] . "\">Edit</a>");
                        $tpl->assign("final",
                            ($lines[$i]['Success'] == 0) ? "<font color=\"red\">Незачет</font> <a href=\"" . $_SERVER['SCRIPT_NAME'] . "?" . student_get_students . "&" . student_get_success . "=" . $lines[$i]['id'] . "\">Ok</a>" : "<font color=\"green\">Зачет</font> <a href=\"" . $_SERVER['SCRIPT_NAME'] . "?" . student_get_students . "&" . student_get_not_success . "=" . $lines[$i]['id'] . "\">X</a>");
                        $tpl->assign("delete",
                            "<a href=\"" . $_SERVER['SCRIPT_NAME'] . "?" . student_get_students . "&" . student_get_delete . "=" . $lines[$i]['id'] . "\">Delete</a>");
                    }
                }
            } else {
            ...
            $tpl->assign("url", $_SERVER['SCRIPT_NAME']);
            $tpl->assign("paramstr", problem_get_problems . "&" . problem_get_add);
            ...
            if (isset($_GET[problem_get_add])) {
                ...
                if (count($lines) > 0) {
                    ...
                    foreach ($lines as $i => $line) {
                        ...
                        $tpl->assign("FAQ", $line['FAQ']);
                        $tpl->assign("url_more",
                            $_SERVER['SCRIPT_NAME'] . "?" . problem_get_problems . "&" . problem_get_id . "=" . $line['id']);
                        $tpl->assign("url_edit",
                            $_SERVER['SCRIPT_NAME'] . "?" . problem_get_problems . "&" . problem_get_edit . "=" . $line['id']);
                        $tpl->assign("url_delete",
                            $_SERVER['SCRIPT_NAME'] . "?" . problem_get_problems . "&" . problem_get_delete . "=" . $line['id']);
                        $tpl->assign("FullDescription", $line['FullDescription']);
                    }
                } else {
